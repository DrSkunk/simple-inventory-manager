...
import { addItem } from '../../src/models/item';
import {
  addGroup,
  updateGroup,
  removeGroup,
...
test('clear items from group', t => {
  const rootGroup = db.getState().groups[0];
  // Clear items on empty item list
  t.deepEqual(rootGroup.items, []);
  clearItemsFromGroup(rootGroup.id);
  t.deepEqual(rootGroup.items, []);

  // Clear two items
  addItem(rootGroup.id, {
    title: 'testItem1',
    description: '',
    barcode: '',
    currency: '',
  ...
  t.is(rootGroup.items[0].title, 'testItem1');
  t.is(rootGroup.items[1].title, 'testItem2');
